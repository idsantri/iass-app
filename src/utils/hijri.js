...
function m2h(inputMasehi, koreksi = 0) {
    ...
    const hasil1 =
        Math.floor(365.25 * mThn) + Math.floor(30.6001 * (mBln + 1)) + mTgl + okt15th1582 - 428;
...
function bacaHijri(input) {
    ...
    const dd = String(input).substring(6, 8);
    const mm = String(input).substring(4, 6);
    const yy = String(input).substring(0, 4);
    if (String(input).length != 8 || dd < 1 || mm < 1 || yy < 1) {
        return 'Format salah!';
    }
